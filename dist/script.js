...
var checkTextInputs = function checkTextInputs(selector) {
  ...
  txtInputs.forEach(function (input) {
    // делаем, чтобы можно было вводить только кириллицу и цифры
    var value = input.value; // и делаем защиту от автозаполнения
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var textMessage = document.createElement('dive');
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+7 (___) ___ __ __',
        ...
        // удаление всех не цифр из матрицы
    ...
    this.value = matrix.replace(/./g, function (a) {
      // перебираем все элементы в матрице и устанавливаем новый value
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    ...
    img.src = img.src.slice(0, -4) + '-1.png'; // из названия картинки удаляем последние 4 символа и ставим свои
...
var scrolling = function scrolling(upSelector) {
  ...
  // 		 prevScrollTop,
  // 		 speed;
  // 	if (to > from) {  // скорость анимации
  // 		speed = 30;
  // 	} else {
  // 		speed = -30;
  // 	}
  // 	let move = setInterval(function() { // делаем анимацию скролла
  // 		let scrollTop = Math.round(body.scrollTop || element.scrollTop);
  // 		if (
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    ...
    }) // передаём массив полученный с сервера в функцию createCards
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      var src = _ref.src,
          title = _ref.title,
          link = _ref.link;
      // перебирается каждый объект находящийся внутри возвращённого массива
